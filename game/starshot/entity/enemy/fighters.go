...
func NewChaser(x, y int) (*Chaser, error) {
	...
	return &Chaser{
		...
		enemyHealth: enemyHealth{hp: chaserMaxHP, maxHP: chaserMaxHP},
...
func (c *Chaser) BoundingBoxOverlaps(other def.Entity) bool {
	return aabbOverlaps(c.x, c.y, c.width, c.height, other)
}
...
func (c *Chaser) Perceive(scene def.Scene) def.Perception {
	return perceiveFighter(c.hp, c.maxHP, chaserLookahead, c.fx, c.fy, c.width, c.height, scene)
}
...
func (c *Chaser) CanBeRemoved() bool { return c.deathComplete() }
...
func (c *Chaser) MarkAsDead(_ def.Scene) { c.startDeath(60) }
...
func NewHunter(x, y int) (*Hunter, error) {
	...
	return &Hunter{
		...
		cachedImg:   ebit.NewImage(w, h),
		pixelBuf:    make([]byte, w*h*4),
		enemyHealth: enemyHealth{hp: hunterMaxHP, maxHP: hunterMaxHP},
...
func (h *Hunter) BoundingBoxOverlaps(other def.Entity) bool {
	return aabbOverlaps(h.x, h.y, h.width, h.height, other)
}
...
func (h *Hunter) Perceive(scene def.Scene) def.Perception {
	return perceiveFighter(h.hp, h.maxHP, hunterLookahead, h.fx, h.fy, h.width, h.height, scene)
}
...
func (h *Hunter) CanBeRemoved() bool { return h.deathComplete() }
...
func (h *Hunter) MarkAsDead(_ def.Scene) { h.startDeath(60) }
